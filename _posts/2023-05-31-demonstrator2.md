...
The development of Vulpex, a vulnerable path exploration through dynamic symbolic execution was motivated by the fact that most fuzzers get stuck relatively fast when fuzzing a Java Application. To this end, the SOTA and RESTler fuzzers are combined with a symbolic execution mechanism, provided by Vulpex, to instrument the Java Byte Code of target web services to find vulnerabilities. As demonstration, the Vulpex Instrumentation Agent was attached to the Java authentication service of crAPI, a vulnerable web application developed by the OWASP team for learning purposes. The results have shown that Vulpex also performs well in comparison to other similar tools such as ShiftLeft Scans and FindSecBugs, which were also mentioned before.
